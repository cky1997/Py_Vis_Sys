...
class MainWindow(QWidget, Ui_Form):
    def __init__(self, parent=None):
        ...
        self.init_widget()
        self.pushButton_6.clicked.connect(self.plot_matwidget)
        ...
        self.labels = [self.wd_1, self.wd_2, self.wd_3, self.wd_4, self.wd_5, self.wd_6]
    ...
    def plot_matwidget(self):
        self.matplotlibwidget.ax.clear()
        # self.matplotlibwidget.axis("off")
        ...
        for index, txt in enumerate(txts):
            ...
            seagreen1_patch = mpatches.Patch(color='#54FF9F', label='Common words in 1 articles')
            turquoise1_patch = mpatches.Patch(color='#00F5FF', label='Common words in 2 articles')
            darkgoldenrod1_patch = mpatches.Patch(color='#FFB90F', label='Common words in 3 articles')
            rosybrown3_patch = mpatches.Patch(color='#CD9B9B', label='Common words in 4 articles')
            red_patch = mpatches.Patch(color='#FF0000', label='Common words in 5 articles')
            self.matplotlibwidget.ax.legend(handles=[seagreen1_patch, turquoise1_patch, darkgoldenrod1_patch, rosybrown3_patch, red_patch], bbox_to_anchor=(1, 1),  loc='upper right')
    ...
    def multi_wordcloud(self):
        ...
        for j in range(len(selected_id_lst)):
            ...
            selected_count_df[j] = pd.merge(selected_count_df[j], inter_text_dict[["words", "appear_count"]], on=["words"], how='left', suffixes=(None, '_x'))
            ...
            selected_count_df[j]["color"] = selected_count_df[j].groupby("appear_count")["appear_count"]. \
                transform(lambda x: "#54FF9F" if (x == 1).all() else ("#00F5FF" if (x==2).all() else ("#FFB90F" if (x==3).all() else ("#CD9B9B" if (x==4).all() else "#FF0000"))))
