...
class Mysignals(QObject):
    '''
    Custom a signal class and create two signal object designed for thread
    '''
...
class MainWindow(QWidget, Ui_Form):
    def __init__(self, parent=None):
        ...
        self.ms.progress.connect(self.progressToGui)

        # Initialize matplotlibwidget
        self.init_widget()

        # Connect MyLabel component to function
        ...
        # 一组词云的文件路径 store a group of file paths of word clouds
        self.wc_file_path = []
        self.wc_file_id = []
    ...
    def progressToGui(self, progress):
    ...
    def multi_wordcloud(self):
        ...
        if len(selected_id_lst) < 2:
            ...
            def run():
                self.progressBar.setRange(0, len(selected_id_lst))
                for j in tqdm(range(len(selected_id_lst))):
                ...
                # Add legend according to the number of articles
                self.plot_matwidget(len(selected_id_lst))
                self.ms.text_print.emit(f"word clouds saved and displayed sucessfully.")
    ...
    def find_similar_articles(self):
        ...
        if len(requested_id) == 0:
            ...
            # Set tableWidget attributes and style sheet
    ...
    def indi_wordcloud(self):
        ...
        if len(selected_id_lst) == 0:
            ...
            # Get a frequency table of common words
    ...
    def detailed_data(self):
        ...
        # Get the line where the selected text is located
        arti_id = self.tableWidget.selectedItems()[0].text()
        # print(arti_id)
        # print(type(row))
